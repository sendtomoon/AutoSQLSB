...
function flushRows() {
	...
	$("#dataTable tbody  tr:odd").find("input").css({
		"background-color" : "#C6E0B4",
		"outline" : "none",
		"border" : "0"
	});
	$("#dataTable tbody  tr:even").find("input").css({
...
function rowValue(filed, type, comment, defval, checked) {
	...
	var html = '<tr height="30px" >' + '<td align="center">' + (len + 1)
			+ '</td>' + '<td><input style="width: 100%; height: 100%" value="'
			+ filed + '" /></td>'
			+ '<td><input style="width: 100%; height: 100%" value="' + type
			+ '" /></td>'
			+ '<td><input style="width: 100%; height: 100%" value="' + comment
			+ '"/></td>'
			+ '<td><input style="width: 100%; height: 100%" value="' + defval
			+ '"/>' + checkStr
